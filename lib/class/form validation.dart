...
class FormValidationFunction {
  ...
  static String? checkPassword(String? s,
      {required bool signUp, required bool login}) {
    if (s == null || s.isEmpty) {
      return 'Cannot be empty';
    }
    if (signUp) {
  ...
  static String? checkAgeField(String? s) {
    if (s == null || s.isEmpty) {
      return 'Cannot be empty';
    }
    if (s.length == 0 || double.parse(s) <= 15 || double.parse(s) >= 120) {
      return ('Age not acceptable');
    }
